...
def add_category():
    ...
    new_category = Category(category_name=data['category_name'], description=data['description'])
...
def update_category(id_category):
    ...
    if 'category_name' in data.keys():
        ...
        if chek.category_name == data['category_name']:
...
def update_medicine(id_medicine):
    ...
    if 'medicine_name' in data.keys():
        ...
        if chek.medicine_name == data['medicine_name']:
            return Response(status=405, response='Invalid input.')
    else:
        if 'medicine_name' in data.keys():
...
def add_order():
    ...
    if datetime.datetime.strptime(data["date_of_purchase"], "%Y-%m-%d %H:%M:%S") >= datetime.datetime.strptime(data["shipData"], "%Y-%m-%d %H:%M:%S"):
        #print(datetime.datetime.strptime(data["date_of_purchase"], "%Y-%m-%d %H:%M:%S"))
        #print(datetime.datetime.strptime(data["shipData"], "%Y-%m-%d %H:%M:%S"))
        return Response(status=405, response='Invalid input.')
    if data["complete"] == 1:
        return Response(status=405, response='Invalid input.')
...
def add_order_medicine():
    ...
    if exists1.order_status == "approved" or exists1.order_status == "delivered" or exists1.complete == 1:
...
def delete_medicine_order(id_order, id_medicine):
    ...
    exists1 = session.query(Order).filter_by(id_order=id_order).first()
    if not exists1:
        return Response(status=405, response='Order with such id does not exist. Invalid input.')

    #if exists1.order_status == "approved" or exists1.order_status == "delivered":
    #    return Response(status=405, response="invalid input")

    deleting = delete(Order_details).where(Order_details.c.order_id == id_order, Order_details.c.medicine_id == id_medicine)
    session.execute(deleting)
    session.commit()
    return Response(status=200, response='Medicine was deleted.')
...
def update_order_details(id_order, id_medicine):
    ...
    exists1 = session.query(Order).filter_by(id_order=id_order).first()
    if not exists1:
        return Response(status=404, response='Order not found.. Invalid input.')
    if exists1.order_status == "approved" or exists1.order_status == "delivered" or exists1.complete == 1:
        return Response(status=405, response="Order approved or delivered or complete")
    exists2 = session.query(Medicine).filter_by(id_medicine=id_medicine).first()
    if not exists2:
        return Response(status=405, response='Medicine with such id does not exist. Invalid input.')
    try:
        Order_detailsSchemaUpdate().load(data)
    except ValidationError as err:
        return jsonify(err.messages), 405
...
def update_order(id_order):
    ...
    if 'user_id' in data.keys():
        chek = session.query(User).filter_by(user_id=data['user_id']).first()
        if not chek:
            return Response(status=405, response='Invalid input.')
    else:
...
def delete_order(id_order):
    ...
    db_order = session.query(Order).filter_by(id_order=id_order).first()
    if not db_order:
        return Response(status=400, response='Invalid value.')
...
def test():
    ...
    test4 = Order(id_order = 1, user_id = 1, address = "test", date_of_purchase = "2022-10-05 18:32:54", shipData=datetime.date.today(), order_status="placed", complete = False)
    ...
    #sql2 = delete(Category).where(Category.id_category == 1)
